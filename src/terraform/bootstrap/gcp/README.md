...
Candidate provider: `hashicorp/google = 6.45.0` (replaces unbounded bootstrap >=4.57).
Reference to verify: https://registry.terraform.io/providers/hashicorp/google/6.45.0/docs/resources/storage_bucket

Explicit dedicated bucket; uniform access, public-access prevention, versioning,
7-day soft-delete and noncurrent history expiration after 90 days/ten newer versions.
Google-managed encryption at rest is default; optional existing `kms_key_name` selects
CMEK. Grant the Cloud Storage service agent the required KMS encrypt/decrypt role
separately, and verify key availability, IAM and rotation/recovery before use.
The explicit controller gets `roles/storage.objectAdmin` on this bucket only,
including native GCS lock object creation/deletion, never a project-level role.
A dedicated bucket is the isolation boundary; the binding is not prefix-scoped.
Doctor compares raw bucket projectNumber against the explicitly described project,
checks protections and public IAM, and reports effective IAM/KMS coverage unknown.
VPC Service Controls/private connectivity require separate controller/CI checks.
