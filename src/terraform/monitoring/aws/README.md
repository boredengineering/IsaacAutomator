# AWS report-only monitoring: Scheduler → CodeBuild

**Decision:** one EventBridge Scheduler universal `codebuild:startBuild` target and
one unprivileged small Linux CodeBuild project. AWS Config/SSM is not provisioned.
Exact provider pin: **hashicorp/aws 5.100.0** (commercial AWS regions only).

Additional explicit inputs: `region`, `vpc_id`, private `subnet_ids`, restricted
`security_group_ids`, and `automator_workdir` (absolute checkout directory baked in
the image). CodeBuild overrides its working directory; the buildspec explicitly
changes to the supplied directory, then invokes the common command. No source is
fetched (`NO_SOURCE`) and no build artifacts/log archives are exported. The image
must live in private ECR, and the existing runtime role supplies ECR pull access.

Scheduler role trust: `scheduler.amazonaws.com`, scoped with source account and the
reviewed schedule group ARN. It needs only `codebuild:StartBuild` on this exact
project ARN (see `scheduler_permission`). Runtime role trust: `codebuild.amazonaws.com`,
constrained by source account/project ARN. Runtime needs exact repository image
pull rights (plus ECR authorization-token API), selected detector reads/locks,
restricted report writes, and CodeBuild VPC ENI permissions restricted to selected
subnets/security groups wherever AWS supports conditions. None are granted here.
Do not share either role with the monitoring administrator.

Runtime concurrency is one build; queue timeout is five minutes; build timeout
must be 300..900 seconds in whole minutes. Scheduler delivery retries, not CodeBuild
execution retries, consume `retries`; event age is capped at 900 seconds. A successful
StartBuild is not a successful check. Private subnets/security groups require
customer-reviewed ECR/backend/report access via endpoints or controlled egress;
this module does not add NAT, routes or public IPs. CloudWatch/S3 build logs are
disabled to avoid accidental private-value retention. Use verified sanitized
external reports and platform execution status for health. Costs include CodeBuild
minutes, scheduler requests, registry/report storage, VPC endpoints/NAT if used.

Official schema/service references checked for this slice:
- https://github.com/hashicorp/terraform-provider-aws/blob/v5.100.0/website/docs/r/codebuild_project.html.markdown
- https://github.com/hashicorp/terraform-provider-aws/blob/v5.100.0/website/docs/r/scheduler_schedule.html.markdown
